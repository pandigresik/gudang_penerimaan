...
<div class="col-md-9"> 
    {!! Form::select('good_receipt_item_weight_id', $goodReceiptItemItems, null, ['class' => 'form-control select2', 'required' => 'required']) !!}
</div>
</div>

@include('warehouse.good_receipt_item_sample_classifications.weighing_item')
@include('warehouse.good_receipt_item_sample_classifications.weighing_result')
...
<script>
    ...
    const validateForm = function(elm){
        ...
        // pastikan totalnya sesuai dengan sample yang dipilih
        ...
        if(_totalTimbang > _targetTimbang){
            _message.push(`Total penimbangan (${_totalTimbang}) tidak boleh lebih besar dari sisa timbang  sample (${_targetTimbang})`)
        }
    ...
    $(function(){
        $('select[name="good_receipt_item_weight_id"]').change(function(){
